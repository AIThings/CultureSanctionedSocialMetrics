...
//		// S1_A1
//		case Financial_A1: return -Math.exp((MitigatedSpeech+loudness+offensiveness)*0.1);
//		
//		// Mitigated Speech + Loudness + Offensiveness
//		// S3_A6
//		case DIGNITY_A6: //return   -(MitigatedSpeech + loudness + offensiveness);
//			return -1 * ImpactFunctionHelper.Heaviside(MitigatedSpeech-4) * Math.exp((MitigatedSpeech+loudness+offensiveness)*0.1); 
//			
//			
//		// Mitigated Speech + Loudness + Offensiveness
//		//S3_A9
...
//			
//		case DIGNITY_A11:
//			double oldValue =  valueSource.getCSSMValue(agentName, new CSSM(CHECK_POINT, DIGNITY, VENDOR, VENDOR, VENDOR, 0, 100,
//					50));
//			if(values.get(0) > 24)
//					return 50 - oldValue;
//			return 0;
...
////			return ImpactFunctionHelper.Signum(5-MitigatedSpeech) * (Math.abs(5-MitigatedSpeech) + ImpactFunctionHelper.Signum(5-MitigatedSpeech)/(loudness));
//			return -1 * ImpactFunctionHelper.Heaviside(MitigatedSpeech-6) * Math.exp((MitigatedSpeech+loudness)*0.1); 
//
//			// 5 * t
...
//			return -1 * ImpactFunctionHelper.Heaviside(MitigatedSpeech-5) * Math.exp((MitigatedSpeech+loudness)*0.1); 
//			
//		//Case when sergeant declines the gift	
//		case POLITENESS_PEER_A9:
//			//return ImpactFunctionHelper.Signum(5-MitigatedSpeech) * (Math.abs(5-MitigatedSpeech) + ImpactFunctionHelper.Signum(5-MitigatedSpeech)/(loudness));
//			return -1 * ImpactFunctionHelper.Heaviside(MitigatedSpeech-3) * Math.exp((MitigatedSpeech)*0.1); 
//			
//		case POLITENESS_PEER_A11:
//			double old_Value =  valueSource.getCSSMValue(agentName, new CSSM(CHECK_POINT, POLITENESS, VENDOR, SERGEANT, VENDOR, 0,
//					100, 50));
//			if(values.get(0) > 24)
//				return 50 - old_Value;
//
